 ...
 * by simulating in-place player behavior and periodically requesting TPAs.
 */
...
const RECONNECT_DELAY = 30000;       // 10 seconds
const RECONNECT_FAIL_DELAY = 30000; // 10 seconds
...
function createAndRunBot(username) {
  ...
  function cleanup() {
    ...
    if (tpaIntervalTimer) clearInterval(tpaIntervalTimer);
  ...
  function handleDisconnect(reason) {
    ...
    if (hasSuccessfullySpawned) {
      ...
      console.log(`[${currentUsername}] Failed to connect. Reconnecting in ${RECONNECT_FAIL_DELAY / 1000 / 60} minutes...`);
  ...
  bot.once('spawn', () => {
    ...
    // Start non-movement idle action cycle
    setTimeout(() => {
